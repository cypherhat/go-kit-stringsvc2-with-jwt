...
func (mw instrumentingMiddleware) Uppercase(ctx context.Context, s string) (output string, err error) {
	defer func(begin time.Time) {
		customClaims := ctx.Value(jwt.JWTClaimsContextKey)
		lvs := []string{"method", "uppercase", "client", customClaims.(basejwt.MapClaims)["clientId"].(string), "error", fmt.Sprint(err != nil)}
...
func (mw instrumentingMiddleware) Count(ctx context.Context, s string) (n int) {
	defer func(begin time.Time) {
		customClaims := ctx.Value(jwt.JWTClaimsContextKey)
		lvs := []string{"method", "uppercase", "client", customClaims.(basejwt.MapClaims)["clientId"].(string), "error", "false"}
		mw.requestCount.With(lvs...).Add(1)
		mw.requestLatency.With(lvs...).Observe(time.Since(begin).Seconds())
